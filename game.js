...
// ── State ────────────────────────────────────────────────────
let playerName   = '';
let playerEmpId  = '';       // Employee ID — used as unique key for one-attempt rule
let playerBU     = '';       // Business Unit (optional)
...
function loadLeaderboard() {
  ...
  lbUnsubscribe = onValue(ref(db, 'leaderboard'), (snap) => {
    if (!snap.exists()) {
    ...
    // Update rank on complete screen
...
window.startGame = async function() {
  ...
  // One-attempt enforcement — check if this EmpID has already played
...
// ── Tab visibility cheat detection ───────────────────────────
// Pauses timer when player switches away (stops tab-switching to look up answers)
let tabSwitchCount = 0;
let timerPaused    = false;
let pausedAt       = null;

document.addEventListener('visibilitychange', () => {
  if (gameComplete || !startTime) return;
  if (document.hidden) {
    // Player switched away — pause timer
    timerPaused = true;
    pausedAt    = Date.now();
    clearInterval(timerInterval);
    tabSwitchCount++;
    ...
    if (timerPaused && pausedAt) {
      startTime += (Date.now() - pausedAt); // shift start time forward by pause duration
      pausedAt   = null;
    }
    timerPaused = false;
    timerInterval = setInterval(() => {
